...
from data_preprocessing import DataPrecessForSentence
...
def model_train_validate_test(train_df, dev_df, test_df, target_dir, 
    ...
    xlnetmodel = XlnetModel(requires_grad = True)
    tokenizer = xlnetmodel.tokenizer
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="max", factor=0.85, patience=0,threshold=1e-8)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i in range(n_classes):
            test_prediction['prob_'+ str(i)] = [item[i] for item in all_prob]
            lst.append('prob_'+ str(i))
        lst.append('prediction')
        test_prediction['prediction'] = test_prediction.apply(lambda x: (np.argmax(x)), axis=1)
        test_prediction = test_prediction[lst]
        test_prediction.to_csv(os.path.join(target_dir,"test_prediction.csv"), index=False)
...
def model_load_test(test_df, target_dir, test_prediction_dir, test_prediction_name, max_seq_len=50, batch_size=32):
    ...
    """
    xlnetmodel = XlnetModel(requires_grad = False)
    tokenizer = xlnetmodel.tokenizer
    device = torch.device("cuda")
    ...
    if not os.path.exists(test_prediction_dir):
        os.makedirs(test_prediction_dir)
    test_prediction.to_csv(os.path.join(test_prediction_dir, test_prediction), index=False)
